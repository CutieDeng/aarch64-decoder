...
(define (int->SWPP i)
  ...
  [else (apply SWPP (int->SWPP/struct i))])
)

(define (SWPP->int swp)
  (match-define (SWPP s a r rt2 o3 opc rn rt) swp)
  (bitwise-ior
...
(struct SWPP (s a r rt2 o3 opc rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LSE128 . #t))
  #:property prop:into-int SWPP->int
  #:property prop:try-from-int int->SWPP
)

(provide (struct-out SWPP))
