...
(define (int->LDP/NEON/Post/struct i)
  (list (bitwise-bit-field i 30 32)
...
(struct LDP/NEON/Post (opc l imm7 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDP/NEON/Post->int
  #:property prop:try-from-int int->LDP/NEON/Post
)
...
(define int->LDP/NEON/Signed/struct int->LDP/NEON/Post/struct)

(define (int->LDP/NEON/Signed i)
  (cond [(nand
    (equal? (bitwise-bit-field i 27 30) #x5)
    (equal? (bitwise-bit-field i 26 27) 1)
...
(struct LDP/NEON/Signed (opc l imm7 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDP/NEON/Signed->int
  #:property prop:try-from-int int->LDP/NEON/Signed
)

(provide (struct-out LDP/NEON/Signed))

(define int->LDNP/NEON/struct int->LDP/NEON/Post/struct)

(define (int->LDNP/NEON i)
  (cond [(nand
    (equal? (bitwise-bit-field i 27 30) #x5)
    (equal? (bitwise-bit-field i 26 27) 1)
...
(struct LDNP/NEON (opc l imm7 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDNP/NEON->int
  #:property prop:try-from-int int->LDNP/NEON
)

(provide (struct-out LDNP/NEON))
