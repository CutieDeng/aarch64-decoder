...
(define int->CPYPRN/struct int->CPYFP/struct)
...
(define (int->CPYPRN i)
  ...
  [else (apply CPYPRN (int->CPYPRN/struct i))])
)

(define (CPYPRN->int rcw)
  (match-define (CPYPRN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPRN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPRN->int
  #:property prop:try-from-int int->CPYPRN
)

(provide (struct-out CPYPRN))

(define int->CPYPRT/struct int->CPYFP/struct)
...
(define (int->CPYPRT i)
  ...
  [else (apply CPYPRT (int->CPYPRT/struct i))])
)

(define (CPYPRT->int rcw)
  (match-define (CPYPRT sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPRT (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPRT->int
  #:property prop:try-from-int int->CPYPRT
)

(provide (struct-out CPYPRT))

(define int->CPYPRTN/struct int->CPYFP/struct)

(define (int->CPYPRTN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPRTN (int->CPYPRTN/struct i))])
)

(define (CPYPRTN->int rcw)
  (match-define (CPYPRTN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPRTN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPRTN->int
  #:property prop:try-from-int int->CPYPRTN
)

(provide (struct-out CPYPRTN))

(define int->CPYPRTRN/struct int->CPYFP/struct)

(define (int->CPYPRTRN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPRTRN (int->CPYPRTRN/struct i))])
)

(define (CPYPRTRN->int rcw)
  (match-define (CPYPRTRN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPRTRN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPRTRN->int
  #:property prop:try-from-int int->CPYPRTRN
)

(provide (struct-out CPYPRTRN))

(define int->CPYPRTWN/struct int->CPYFP/struct)

(define (int->CPYPRTWN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPRTWN (int->CPYPRTWN/struct i))])
)

(define (CPYPRTWN->int rcw)
  (match-define (CPYPRTWN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPRTWN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPRTWN->int
  #:property prop:try-from-int int->CPYPRTWN
)

(provide (struct-out CPYPRTWN))

(define int->CPYPT/struct int->CPYFP/struct)
...
(define (int->CPYPT i)
  ...
  [else (apply CPYPT (int->CPYPT/struct i))])
)

(define (CPYPT->int rcw)
  (match-define (CPYPT sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPT (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPT->int
  #:property prop:try-from-int int->CPYPT
)

(provide (struct-out CPYPT))

(define int->CPYPTN/struct int->CPYFP/struct)
...
(define (int->CPYPTN i)
  ...
  [else (apply CPYPTN (int->CPYPTN/struct i))])
)

(define (CPYPTN->int rcw)
  (match-define (CPYPTN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPTN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPTN->int
  #:property prop:try-from-int int->CPYPTN
)

(provide (struct-out CPYPTN))

(define int->CPYPTRN/struct int->CPYFP/struct)

(define (int->CPYPTRN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPTRN (int->CPYPTRN/struct i))])
)

(define (CPYPTRN->int rcw)
  (match-define (CPYPTRN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPTRN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPTRN->int
  #:property prop:try-from-int int->CPYPTRN
)

(provide (struct-out CPYPTRN))

(define int->CPYPTWN/struct int->CPYFP/struct)

(define (int->CPYPTWN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPTWN (int->CPYPTWN/struct i))])
)

(define (CPYPTWN->int rcw)
  (match-define (CPYPTWN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPTWN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPTWN->int
  #:property prop:try-from-int int->CPYPTWN
)

(provide (struct-out CPYPTWN))

(define int->CPYPWN/struct int->CPYFP/struct)
...
(define (int->CPYPWN i)
  ...
  [else (apply CPYPWN (int->CPYPWN/struct i))])
)

(define (CPYPWN->int rcw)
  (match-define (CPYPWN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPWN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPWN->int
  #:property prop:try-from-int int->CPYPWN
)

(provide (struct-out CPYPWN))

(define int->CPYPWT/struct int->CPYFP/struct)
...
(define (int->CPYPWT i)
  ...
  [else (apply CPYPWT (int->CPYPWT/struct i))])
)

(define (CPYPWT->int rcw)
  (match-define (CPYPWT sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPWT (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPWT->int
  #:property prop:try-from-int int->CPYPWT
)

(provide (struct-out CPYPWT))

(define int->CPYPWTN/struct int->CPYFP/struct)

(define (int->CPYPWTN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPWTN (int->CPYPWTN/struct i))])
)

(define (CPYPWTN->int rcw)
  (match-define (CPYPWTN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPWTN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPWTN->int
  #:property prop:try-from-int int->CPYPWTN
)
...
(define int->CPYPWTWN/struct int->CPYFP/struct)

(define (int->CPYPWTWN i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 27 30) #x3)
    (equal? (bitwise-bit-field i 26 27) #x1)
    (equal? (bitwise-bit-field i 24 26) #x1)
  ...
  [else (apply CPYPWTWN (int->CPYPWTWN/struct i))])
)

(define (CPYPWTWN->int rcw)
  (match-define (CPYPWTWN sz op1 rs op2 rn rd) rcw)
  (bitwise-ior
    (arithmetic-shift sz 30)
    (arithmetic-shift #x3 27)
    (arithmetic-shift #x1 26)
...
(struct CPYPWTWN (sz op1 rs op2 rn rd)
  #:transparent
  #:property prop:in-feature 'FEAT_MOPS
  #:property prop:into-int CPYPWTWN->int
  #:property prop:try-from-int int->CPYPWTWN
)

(provide (struct-out CPYPWTWN))
