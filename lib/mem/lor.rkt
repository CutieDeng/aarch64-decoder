#lang racket

(require "../util/in-feature.rkt")
(require "../util/into-int.rkt")
(require "../util/try-from-int.rkt")

(define (int->LDLARB/struct i)
  (list (bitwise-bit-field i 30 32)
    (bitwise-bit-field i 22 23)
    (bitwise-bit-field i 16 21)
    (bitwise-bit-field i 15 16)
    (bitwise-bit-field i 10 15)
    (bitwise-bit-field i 5 10)
    (bitwise-bit-field i 0 5))
)

(define (int->LDLARB i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 0)
    (equal? (bitwise-bit-field i 24 30) #x8)
    (equal? (bitwise-bit-field i 23 24) #x1)
    (equal? (bitwise-bit-field i 22 23) 1)
    (equal? (bitwise-bit-field i 21 22) 0)
    (equal? (bitwise-bit-field i 16 21) #x1f)
    (equal? (bitwise-bit-field i 15 16) 0)
    (equal? (bitwise-bit-field i 10 15) #x1f)
  ) #f]
  [else (apply LDLARB (int->LDLARB/struct i))])
)

(define (LDLARB->int l)
  (match-define (LDLARB size l rs o0 rt2 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x8 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift l 22)
    (arithmetic-shift rs 16)
    (arithmetic-shift o0 15)
    (arithmetic-shift rt2 10)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct LDLARB (size l rs o0 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LOR . #t))
  #:property prop:into-int LDLARB->int
  #:property prop:try-from-int int->LDLARB
)

(provide (struct-out LDLARB))

(define int->LDLARH/struct int->LDLARB/struct)

(define (int->LDLARH i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 1)
    (equal? (bitwise-bit-field i 24 30) #x8)
    (equal? (bitwise-bit-field i 23 24) #x1)
    (equal? (bitwise-bit-field i 22 23) 1)
    (equal? (bitwise-bit-field i 21 22) 0)
    (equal? (bitwise-bit-field i 16 21) #x1f)
    (equal? (bitwise-bit-field i 15 16) 0)
    (equal? (bitwise-bit-field i 10 15) #x1f)
  ) #f]
  [else (apply LDLARH (int->LDLARH/struct i))])
)

(define (LDLARH->int l)
  (match-define (LDLARH size l rs o0 rt2 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x8 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift l 22)
    (arithmetic-shift rs 16)
    (arithmetic-shift o0 15)
    (arithmetic-shift rt2 10)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct LDLARH (size l rs o0 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LOR . #t))
  #:property prop:into-int LDLARH->int
  #:property prop:try-from-int int->LDLARH
)

(provide (struct-out LDLARH))

(define int->LDLAR/struct int->LDLARB/struct)

(define (int->LDLAR i)
  (cond [(nand (equal? (bitwise-bit-field i 31 32) 1)
    (equal? (bitwise-bit-field i 24 30) #x8)
    (equal? (bitwise-bit-field i 23 24) #x1)
    (equal? (bitwise-bit-field i 22 23) 1)
    (equal? (bitwise-bit-field i 21 22) 0)
    (equal? (bitwise-bit-field i 16 21) #x1f)
    (equal? (bitwise-bit-field i 15 16) 0)
    (equal? (bitwise-bit-field i 10 15) #x1f)
  ) #f]
  [else (apply LDLAR (int->LDLAR/struct i))])
)

(define (LDLAR->int l)
  (match-define (LDLAR size l rs o0 rt2 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x8 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift l 22)
    (arithmetic-shift rs 16)
    (arithmetic-shift o0 15)
    (arithmetic-shift rt2 10)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct LDLAR (size l rs o0 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LOR . #t))
  #:property prop:into-int LDLAR->int
  #:property prop:try-from-int int->LDLAR
)

(provide (struct-out LDLAR))

(define int->STLLRB/struct int->LDLARB/struct)

(define (int->STLLRB i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 0)
    (equal? (bitwise-bit-field i 24 30) #x8)
    (equal? (bitwise-bit-field i 23 24) #x1)
    (equal? (bitwise-bit-field i 22 23) 0)
    (equal? (bitwise-bit-field i 21 22) 0)
    (equal? (bitwise-bit-field i 16 21) #x1f)
    (equal? (bitwise-bit-field i 15 16) 0)
    (equal? (bitwise-bit-field i 10 15) #x1f)
  ) #f]
  [else (apply STLLRB (int->STLLRB/struct i))])
)

(define (STLLRB->int l)
  (match-define (STLLRB size l rs o0 rt2 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x8 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift l 22)
    (arithmetic-shift rs 16)
    (arithmetic-shift o0 15)
    (arithmetic-shift rt2 10)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct STLLRB (size l rs o0 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LOR . #t))
  #:property prop:into-int STLLRB->int
  #:property prop:try-from-int int->STLLRB
)

(provide (struct-out STLLRB))

(define int->STLLRH/struct int->LDLARB/struct)

(define (int->STLLRH i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 1)
    (equal? (bitwise-bit-field i 24 30) #x8)
    (equal? (bitwise-bit-field i 23 24) #x1)
    (equal? (bitwise-bit-field i 22 23) 0)
    (equal? (bitwise-bit-field i 21 22) 0)
    (equal? (bitwise-bit-field i 16 21) #x1f)
    (equal? (bitwise-bit-field i 15 16) 0)
    (equal? (bitwise-bit-field i 10 15) #x1f)
  ) #f]
  [else (apply STLLRH (int->STLLRH/struct i))])
)

(define (STLLRH->int l)
  (match-define (STLLRH size l rs o0 rt2 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x8 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift l 22)
    (arithmetic-shift rs 16)
    (arithmetic-shift o0 15)
    (arithmetic-shift rt2 10)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct STLLRH (size l rs o0 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LOR . #t))
  #:property prop:into-int STLLRH->int
  #:property prop:try-from-int int->STLLRH
)

(provide (struct-out STLLRH))

(define int->STLLR/struct int->LDLARB/struct)

(define (int->STLLR i)
  (cond [(nand (equal? (bitwise-bit-field i 31 32) 1)
    (equal? (bitwise-bit-field i 24 30) #x8)
    (equal? (bitwise-bit-field i 23 24) #x1)
    (equal? (bitwise-bit-field i 22 23) 0)
    (equal? (bitwise-bit-field i 21 22) 0)
    (equal? (bitwise-bit-field i 16 21) #x1f)
    (equal? (bitwise-bit-field i 15 16) 0)
    (equal? (bitwise-bit-field i 10 15) #x1f)
  ) #f]
  [else (apply STLLR (int->STLLR/struct i))])
)

(define (STLLR->int l)
  (match-define (STLLR size l rs o0 rt2 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x8 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift l 22)
    (arithmetic-shift rs 16)
    (arithmetic-shift o0 15)
    (arithmetic-shift rt2 10)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct STLLR (size l rs o0 rt2 rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LOR . #t))
  #:property prop:into-int STLLR->int
  #:property prop:try-from-int int->STLLR
)

(provide (struct-out STLLR))
