...
(define int->STURH/struct int->STUR/struct)
...
(define (int->STURH i)
  ...
  [else (apply STURH (int->STURH/struct i))])
)

(define (STURH->int l)
  (match-define (STURH size opc imm9 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x7 27)
    (arithmetic-shift opc 22)
...
(struct STURH (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int STURH->int
  #:property prop:try-from-int int->STURH
)

(provide (struct-out STURH))
