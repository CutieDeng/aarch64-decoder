...
(define (int->LDFF1D i)
  ...
  [else (apply LDFF1D (int->LDFF1D/struct i))])
)

(define (LDFF1D->int ld1)
  (match-define (LDFF1D dtype rm pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift rm 16)
...
(struct LDFF1D (dtype rm pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDFF1D->int
  #:property prop:try-from-int int->LDFF1D
)

(provide (struct-out LDFF1D))

(define int->LDFF1H/struct int->LDFF1B/b/struct)
...
(define (int->LDFF1H i)
  ...
  [else (apply LDFF1H (int->LDFF1H/struct i))])
)

(define (LDFF1H->int ld1)
  (match-define (LDFF1H dtype rm pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift rm 16)
...
(struct LDFF1H (dtype rm pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDFF1H->int
  #:property prop:try-from-int int->LDFF1H
)

(provide (struct-out LDFF1H))

(define int->LDFF1SB/struct int->LDFF1B/b/struct)

(define (int->LDFF1SB i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    (equal? (bitwise-bit-field i 21 25) #xe)
    (equal? (bitwise-bit-field i 13 16) #x3)
  ) #f]
  [else (apply LDFF1SB (int->LDFF1SB/struct i))])
)

(define (LDFF1SB->int ld1)
  (match-define (LDFF1SB dtype rm pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift rm 16)
...
(struct LDFF1SB (dtype rm pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDFF1SB->int
  #:property prop:try-from-int int->LDFF1SB
)

(provide (struct-out LDFF1SB))

(define int->LDFF1SH/struct int->LDFF1B/b/struct)

(define (int->LDFF1SH i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    (equal? (bitwise-bit-field i 21 25) #x9)
    (equal? (bitwise-bit-field i 13 16) #x3)
  ) #f]
  [else (apply LDFF1SH (int->LDFF1SH/struct i))])
)

(define (LDFF1SH->int ld1)
  (match-define (LDFF1SH dtype rm pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift rm 16)
...
(struct LDFF1SH (dtype rm pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDFF1SH->int
  #:property prop:try-from-int int->LDFF1SH
)

(provide (struct-out LDFF1SH))

(define int->LDFF1SW/struct int->LDFF1B/b/struct)

(define (int->LDFF1SW i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    (equal? (bitwise-bit-field i 21 25) #x4)
    (equal? (bitwise-bit-field i 13 16) #x3)
  ) #f]
  [else (apply LDFF1SW (int->LDFF1SW/struct i))])
)

(define (LDFF1SW->int ld1)
  (match-define (LDFF1SW dtype rm pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift rm 16)
...
(struct LDFF1SW (dtype rm pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDFF1SW->int
  #:property prop:try-from-int int->LDFF1SW
)

(provide (struct-out LDFF1SW))

(define int->LDFF1W/struct int->LDFF1B/b/struct)
...
(define (int->LDFF1W i)
  ...
  [else (apply LDFF1W (int->LDFF1W/struct i))])
)

(define (LDFF1W->int ld1)
  (match-define (LDFF1W dtype rm pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift rm 16)
...
(struct LDFF1W (dtype rm pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDFF1W->int
  #:property prop:try-from-int int->LDFF1W
)

(provide (struct-out LDFF1W))

(define int->LDNF1B/struct int->LD1SB/struct)
...
(define (int->LDNF1B i)
  ...
  [else (apply LDNF1B (int->LDNF1B/struct i))])
)

(define (LDNF1B->int ld1)
  (match-define (LDNF1B dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
...
(struct LDNF1B (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1B->int
  #:property prop:try-from-int int->LDNF1B
)

(provide (struct-out LDNF1B))

(define int->LDNF1D/struct int->LD1SB/struct)
...
(define (int->LDNF1D i)
  ...
  [else (apply LDNF1D (int->LDNF1D/struct i))])
)

(define (LDNF1D->int ld1)
  (match-define (LDNF1D dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift #x1 20)
...
(struct LDNF1D (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1D->int
  #:property prop:try-from-int int->LDNF1D
)

(provide (struct-out LDNF1D))

(define int->LDNF1H/struct int->LD1SB/struct)
...
(define (int->LDNF1H i)
  ...
  [else (apply LDNF1H (int->LDNF1H/struct i))])
)

(define (LDNF1H->int ld1)
  (match-define (LDNF1H dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift #x1 20)
...
(struct LDNF1H (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1H->int
  #:property prop:try-from-int int->LDNF1H
)

(provide (struct-out LDNF1H))

(define int->LDNF1SB/struct int->LD1SB/struct)

(define (int->LDNF1SB i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    ((or/c #xe #xd #xc) (bitwise-bit-field i 21 25))
    (equal? (bitwise-bit-field i 20 21) #x1)
    (equal? (bitwise-bit-field i 13 16) #x5)
  ) #f]
  [else (apply LDNF1SB (int->LDNF1SB/struct i))])
)

(define (LDNF1SB->int ld1)
  (match-define (LDNF1SB dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift #x1 20)
...
(struct LDNF1SB (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1SB->int
  #:property prop:try-from-int int->LDNF1SB
)

(provide (struct-out LDNF1SB))

(define int->LDNF1SH/struct int->LD1SB/struct)

(define (int->LDNF1SH i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    ((or/c #x9 #x8) (bitwise-bit-field i 21 25))
    (equal? (bitwise-bit-field i 20 21) #x1)
    (equal? (bitwise-bit-field i 13 16) #x5)
  ) #f]
  [else (apply LDNF1SH (int->LDNF1SH/struct i))])
)

(define (LDNF1SH->int ld1)
  (match-define (LDNF1SH dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift #x1 20)
...
(struct LDNF1SH (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1SH->int
  #:property prop:try-from-int int->LDNF1SH
)

(provide (struct-out LDNF1SH))

(define int->LDNF1SW/struct int->LD1SB/struct)

(define (int->LDNF1SW i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    ((or/c #x4) (bitwise-bit-field i 21 25))
    (equal? (bitwise-bit-field i 20 21) #x1)
    (equal? (bitwise-bit-field i 13 16) #x5)
  ) #f]
  [else (apply LDNF1SW (int->LDNF1SW/struct i))])
)

(define (LDNF1SW->int ld1)
  (match-define (LDNF1SW dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift #x1 20)
...
(struct LDNF1SW (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1SW->int
  #:property prop:try-from-int int->LDNF1SW
)

(provide (struct-out LDNF1SW))

(define int->LDNF1W/struct int->LD1SB/struct)

(define (int->LDNF1W i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 25 32) #x52)
    ((or/c #xa #xb) (bitwise-bit-field i 21 25))
    (equal? (bitwise-bit-field i 20 21) #x1)
    (equal? (bitwise-bit-field i 13 16) #x5)
  ) #f]
  [else (apply LDNF1W (int->LDNF1W/struct i))])
)

(define (LDNF1W->int ld1)
  (match-define (LDNF1W dtype imm4 pg rn zt) ld1)
  (bitwise-ior
    (arithmetic-shift #x52 25)
    (arithmetic-shift dtype 21)
    (arithmetic-shift #x1 20)
...
(struct LDNF1W (dtype imm4 pg rn zt)
  #:transparent
  #:property prop:in-feature 'FEAT_SVE
  #:property prop:into-int LDNF1W->int
  #:property prop:try-from-int int->LDNF1W
)

(provide (struct-out LDNF1W))
