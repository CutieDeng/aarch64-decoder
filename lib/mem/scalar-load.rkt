...
(define int->LDRSH/i/Post/struct int->LDRSB/i/Post/struct)

(define (int->LDRSH/i/Post i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 1)
    (equal? (bitwise-bit-field i 27 30) #x7)
    (equal? (bitwise-bit-field i 26 27) 0)
    (equal? (bitwise-bit-field i 24 26) 0)
...
(struct LDRSH/i/Post (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSH/i/Post->int
  #:property prop:try-from-int int->LDRSH/i/Post
)

(provide (struct-out LDRSH/i/Post))

(define int->LDRSH/i/Pre/struct int->LDRSB/i/Post/struct)

(define (int->LDRSH/i/Pre i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 1)
    (equal? (bitwise-bit-field i 27 30) #x7)
    (equal? (bitwise-bit-field i 26 27) 0)
    (equal? (bitwise-bit-field i 24 26) 0)
...
(struct LDRSH/i/Pre (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSH/i/Pre->int
  #:property prop:try-from-int int->LDRSH/i/Pre
)

(provide (struct-out LDRSH/i/Pre))

(define int->LDRSH/i/Unsigned/struct int->LDRSB/i/Post/struct)

(define (int->LDRSH/i/Unsigned i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) 1)
    (equal? (bitwise-bit-field i 27 30) #x7)
    (equal? (bitwise-bit-field i 26 27) 0)
    (equal? (bitwise-bit-field i 24 26) 1)
...
(struct LDRSH/i/Unsigned (size opc imm12 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSH/i/Unsigned->int
  #:property prop:try-from-int int->LDRSH/i/Unsigned
)

(provide (struct-out LDRSH/i/Unsigned))

(define int->LDRSW/r/struct int->LDRH/r/struct)

(define (int->LDRSW/r i)
...
(struct LDRSW/r (size opc rm option s rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSW/r->int
  #:property prop:try-from-int int->LDRSW/r
)

(provide (struct-out LDRSW/r))

(define int->LDRSW/i/Post/struct int->LDRSB/i/Post/struct)

(define (int->LDRSW/i/Post i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) #x2)
    (equal? (bitwise-bit-field i 27 30) #x7)
    (equal? (bitwise-bit-field i 26 27) 0)
    (equal? (bitwise-bit-field i 24 26) 0)
...
(struct LDRSW/i/Post (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSW/i/Post->int
  #:property prop:try-from-int int->LDRSW/i/Post
)

(provide (struct-out LDRSW/i/Post))

(define int->LDRSW/i/Pre/struct int->LDRSB/i/Post/struct)

(define (int->LDRSW/i/Pre i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) #x2)
    (equal? (bitwise-bit-field i 27 30) #x7)
    (equal? (bitwise-bit-field i 26 27) 0)
    (equal? (bitwise-bit-field i 24 26) 0)
...
(struct LDRSW/i/Pre (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSW/i/Pre->int
  #:property prop:try-from-int int->LDRSW/i/Pre
)

(provide (struct-out LDRSW/i/Pre))

(define int->LDRSW/i/Unsigned/struct int->LDRSB/i/Post/struct)

(define (int->LDRSW/i/Unsigned i)
  (cond [(nand (equal? (bitwise-bit-field i 30 32) #x2)
    (equal? (bitwise-bit-field i 27 30) #x7)
    (equal? (bitwise-bit-field i 26 27) 0)
    (equal? (bitwise-bit-field i 24 26) 1)
...
(struct LDRSW/i/Unsigned (size opc imm12 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDRSW/i/Unsigned->int
  #:property prop:try-from-int int->LDRSW/i/Unsigned
)
