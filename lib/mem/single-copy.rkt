...
(define (int->LD64BV i)
  ...
  [else (apply LD64BV (int->LD64BV/struct i))])
)

(define (LD64BV->int ldadd)
  (match-define (LD64BV rs rn rt) ldadd)
  (bitwise-ior
    (arithmetic-shift #x3 30)
    (arithmetic-shift #x7 27)
    (arithmetic-shift 1 21)
    ...
    (arithmetic-shift 1 15)
    (arithmetic-shift #x3 12)
    (arithmetic-shift rn 5)
    rt
  )
)

(struct LD64BV (rs rn rt)
  #:transparent
  #:property prop:in-feature #hash((FEAT_LS64_V . #t))
  #:property prop:into-int LD64BV->int
  #:property prop:try-from-int int->LD64BV
)
