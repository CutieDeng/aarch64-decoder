...
(define int->LD4/s/Post/struct int->LD1/s/Post/struct)

(define (int->LD4/s/Post i)
  (cond [(nand
    (equal? (bitwise-bit-field i 31 32) 0)
    (equal? (bitwise-bit-field i 23 30) #x1b)
    (equal? (bitwise-bit-field i 22 23) 1)
  ...
  [else (apply LD4/s/Post (int->LD4/s/Post/struct i))])
)

(define (LD4/s/Post->int ld4)
  (match-define (LD4/s/Post q l r rm opcode s size rn rt) ld4)
  (bitwise-ior
    (arithmetic-shift q 30)
    (arithmetic-shift #x1b 23)
    (arithmetic-shift l 22)
...
(struct LD4/s/Post (q l r rm opcode s size rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LD4/s/Post->int
  #:property prop:try-from-int int->LD4/s/Post
)

(provide (struct-out LD4/s/Post))

(define int->LD4/m/struct int->LD1/m/struct)
...
(define (int->LD4/m i)
  ...
  [else (apply LD4/m (int->LD4/m/struct i))])
)

(define (LD4/m->int ld4)
  (match-define (LD4/m q l opcode size rn rt) ld4)
  (bitwise-ior
    (arithmetic-shift q 30)
    (arithmetic-shift #x18 23)
    (arithmetic-shift l 22)
...
(struct LD4/m (q l opcode size rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LD4/m->int
  #:property prop:try-from-int int->LD4/m
)

(provide (struct-out LD4/m))

(define int->LD4/m/Post/struct int->LD1/m/Post/struct)

(define (int->LD4/m/Post i)
  (cond [(nand
    (equal? (bitwise-bit-field i 31 32) 0)
    (equal? (bitwise-bit-field i 23 30) #x19)
    (equal? (bitwise-bit-field i 22 23) 1)
...
(define (LD4/m/Post->int ld4)
  (match-define (LD4/m/Post q l rm opcode size rn rt) ld4)
  (bitwise-ior
    (arithmetic-shift q 30)
    (arithmetic-shift #x19 23)
    (arithmetic-shift l 22)
...
(struct LD4/m/Post (q l rm opcode size rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LD4/m/Post->int
  #:property prop:try-from-int int->LD4/m/Post
)

(provide (struct-out LD4/m/Post))
