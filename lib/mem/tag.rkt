...
(define (int->STGP/Post i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 31 32) #x0)
    (equal? (bitwise-bit-field i 30 31) #x1)
    (equal? (bitwise-bit-field i 27 30) #x5)
    (equal? (bitwise-bit-field i 26 27) #x0)
    (equal? (bitwise-bit-field i 23 26) #x1)
    (equal? (bitwise-bit-field i 22 23) #x0)
  ) #f]
  [else (apply STGP/Post (int->STGP/Post/struct i))])
)

(define (STGP/Post->int rcw)
  (match-define (STGP/Post simm7 xt2 xn xt) rcw)
  (bitwise-ior
...
(struct STGP/Post (simm7 xt2 xn xt)
  #:transparent
  #:property prop:in-feature 'FEAT_MTE
  #:property prop:into-int STGP/Post->int
  #:property prop:try-from-int int->STGP/Post
)
...
(define int->STGP/Signed/struct int->STGP/Post/struct)

(define (int->STGP/Signed i)
  (cond [(nand 
    (equal? (bitwise-bit-field i 31 32) #x0)
    (equal? (bitwise-bit-field i 30 31) #x1)
    (equal? (bitwise-bit-field i 27 30) #x5)
  ...
  [else (apply STGP/Signed (int->STGP/Signed/struct i))])
)

(define (STGP/Signed->int rcw)
  (match-define (STGP/Signed simm7 xt2 xn xt) rcw)
  (bitwise-ior
    (arithmetic-shift #x1 30)
    (arithmetic-shift #x5 27)
...
(struct STGP/Signed (simm7 xt2 xn xt)
  #:transparent
  #:property prop:in-feature 'FEAT_MTE
  #:property prop:into-int STGP/Signed->int
  #:property prop:try-from-int int->STGP/Signed
)
...
(define (int->LDGM i)
  ...
  [else (apply LDGM (int->LDGM/struct i))])
)

(define (LDGM->int rcw)
  (match-define (LDGM xn xt) rcw)
  (bitwise-ior
    (arithmetic-shift #xd9 24)
    (arithmetic-shift #x1 23)
    (arithmetic-shift #x1 22)
...
(struct LDGM (xn xt)
  ...
  #:property prop:into-int LDGM->int
  #:property prop:try-from-int int->LDGM
)

(provide (struct-out LDGM))

(define int->STGM/struct int->LDGM/struct)
...
(define int->STZGM/struct int->LDGM/struct)
...
(define (int->STZGM i)
  ...
  [else (apply STZGM (int->STZGM/struct i))])
)

(define (STZGM->int rcw)
  (match-define (STZGM xn xt) rcw)
  (bitwise-ior
    (arithmetic-shift #xd9 24)
    (arithmetic-shift #x1 21)
    (arithmetic-shift xn 5)
...
(struct STZGM (xn xt)
  #:transparent
  #:property prop:in-feature 'FEAT_MTE2
  #:property prop:into-int STZGM->int
  #:property prop:try-from-int int->STZGM
)

(provide (struct-out STZGM))
