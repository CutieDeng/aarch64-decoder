...
(define int->LDTRSH/struct int->LDTR/struct)
...
(define (int->LDTRSH i)
  ...
  [else (apply LDTRSH (int->LDTRSH/struct i))])
)

(define (LDTRSH->int l)
  (match-define (LDTRSH size opc imm9 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x7 27)
    (arithmetic-shift opc 22)
...
(struct LDTRSH (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDTRSH->int
  #:property prop:try-from-int int->LDTRSH
)

(provide (struct-out LDTRSH))

(define int->LDTRSW/struct int->LDTR/struct)
...
(define (int->LDTRSW i)
  ...
  [else (apply LDTRSW (int->LDTRSW/struct i))])
)

(define (LDTRSW->int l)
  (match-define (LDTRSW size opc imm9 rn rt) l)
  (bitwise-ior
    (arithmetic-shift size 30)
    (arithmetic-shift #x7 27)
    (arithmetic-shift opc 22)
...
(struct LDTRSW (size opc imm9 rn rt)
  #:transparent
  #:property prop:in-feature #hash()
  #:property prop:into-int LDTRSW->int
  #:property prop:try-from-int int->LDTRSW
)

(provide (struct-out LDTRSW))
